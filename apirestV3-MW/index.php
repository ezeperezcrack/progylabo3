<?php
use \Psr\Http\Message\ServerRequestInterface as Request;// alias de -interfaces- estandarizadas en el Psr
use \Psr\Http\Message\ResponseInterface as Response;//<- es inmutable, siempre es el mismo. aunque se modifique sigue siendo el mismo. No se puede crear uno nuevo
...
/* FUNCION MIDDLEWARE - el next es un callback*/
$VerificadorDeCredenciales = function ($request, $response, $next) {

  if($request->isGet())
  {
     $response->getBody()->write('<p>NO necesita credenciales para los get</p>');
     $response = $next($request, $response);//abre el paso para el request, callback
  }
  else
  {
...
$app->add(function ($request, $response, $next) {
  $ipvisitante = $_SERVER["REMOTE_ADDR"];
  $datos= "ip: ".$ipvisitante;
  $miArchivo=fopen("datosCliente.txt","a");
  fwrite($miArchivo, $datos."\r\t");
});
...
})->add($VerificadorDeCredenciales);//pertenece a cd se le asigna al middleware mediante la flecha 
